...
const props = defineProps({
  ascensor: Object,     // { id, codigo_interno, edificio, direccion, numero_ascensor, descripcion, estado, qr_slug }
  revisiones: Array,    // últimas revisiones
})

const publicUrl = computed(() => `/a/${props.ascensor.qr_slug}`)
    ...
    <div class="py-8">
      <div class="max-w-6xl mx-auto sm:px-6 lg:px-8 space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div class="md:col-span-2 bg-white rounded shadow p-6 space-y-3">
            ...
            <div class="grid sm:grid-cols-2 gap-4 text-sm">
              <div>
                ...
                <div class="font-medium">{{ ascensor.codigo_interno }}</div>
              </div>
              <div>
                <div class="text-gray-500">N° de ascensor</div>
                <div class="font-medium">{{ ascensor.numero_ascensor || '—' }}</div>
              </div>
              <div>
                <div class="text-gray-500">Edificio</div>
                <div class="font-medium">{{ ascensor.edificio || '—' }}</div>
              </div>
              <div>
                <div class="text-gray-500">Dirección</div>
                <div class="font-medium">{{ ascensor.direccion || '—' }}</div>
              </div>
            </div>
            <div>
          ...
          <!-- QR simple usando servicio público (opcional) -->
